...
static const char *branchmiss_usage = "Branch Misprediction Anomaly (FIXED - no mispredictions).\n\n"
...
int branchmiss(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "hvs:d:t:r:p:", long_opt, NULL)) != -1) {
        switch (opt) {
            ...
            case 'h':
            default:
                printf("Usage: hpas_fixed branchmiss [OPTIONS]\n%s", branchmiss_usage);
                return 0;
        }
    }
    ...
    printf("Starting branchmiss (FIXED): n=%d (%.0f MB/array), procs=%d, duration=%.1f\n",
