...
static const char *precwaste_usage = "Precision Waste / Scalar MMM Anomaly.\n\n"
...
static void do_mmm(const double *A, const double *B, double *C, int n) {
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            for (int k = 0; k < n; k++)
...
static void precwaste_worker(const double *A, const double *B, double *C,
...
int precwaste(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "hvn:d:t:r:p:", long_opt, NULL)) != -1) {
        switch (opt) {
            ...
            case 'h':
            default:
                printf("Usage: hpas precwaste [OPTIONS]\n%s", precwaste_usage);
                return 0;
        }
    }
    ...
    printf("Starting precwaste: n=%d (%ld MB/matrix), procs=%d, duration=%.1f\n",
           n, ((long)n * n * sizeof(double)) / (1024 * 1024), nprocs, duration);
    fflush(stdout);
